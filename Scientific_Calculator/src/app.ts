...
function trignoCalc(): void {
    if (expression.includes(')')) {
        ...
        const num: string = expression.substring(4, expression.length);
        if (expression.includes('sin')) {
            expression = Math.sin(parseFloat(num)).toString();
        }
        else if (expression.includes('cos')) {
...
// function for checking isNaN   
interface forNaN {
    (exp: string, val?: number): void
}

const forNaN: forNaN = (exp) => {
